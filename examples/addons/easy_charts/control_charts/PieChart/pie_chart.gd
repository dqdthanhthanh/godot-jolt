...
var focused_slice: PackedVector2Array
...
func _draw_pie() -> void:
	for i in _slices_polygons.size():
		...
		draw_polyline(_slices_polygons[i] + PackedVector2Array([_slices_polygons[i][0]]), Color.WHITE, 2.0, true)
...
func _input(event: InputEvent) -> void:
	if event is InputEventMouse:
		for slice_i in _slices_polygons.size():
			var slice: PackedVector2Array = _slices_polygons[slice_i]
			if Geometry2D.is_point_in_polygon(event.position, slice):
		...
		focused_slice = PackedVector2Array()
		$Tooltip.hide()
