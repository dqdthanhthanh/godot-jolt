...
func _draw_background() -> void:
	draw_rect(frame,chart_properties.colors.background,true,1.0)# false) TODOGODOT4 Antialiasing argument is missing
	
#	# (debug)
#	var half: Vector2 = node_box.size / 2
#	draw_line(Vector2(half.x, node_box.position.y),Vector2(half.x, node_box.size.y),Color.RED,3)
#	draw_line(Vector2(node_box.position.x, half.y),Vector2(node_box.size.x, half.y),Color.RED,3)


func _draw_borders() -> void:
	draw_rect(node_box,Color.RED,false,1)# true) TODOGODOT4 Antialiasing argument is missing

func _draw_bounding_box() -> void:
	draw_rect(bounding_box,chart_properties.colors.bounding_box,false,1)# true) TODOGODOT4 Antialiasing argument is missing
	
#	# (debug)
#	var half: Vector2 = (bounding_box.size) / 2
#	draw_line(bounding_box.position + Vector2(half.x, 0),bounding_box.position + Vector2(half.x, bounding_box.size.y),Color.RED,3)
#	draw_line(bounding_box.position + Vector2(0, half.y),bounding_box.position + Vector2(bounding_box.size.x, half.y),Color.RED,3)
