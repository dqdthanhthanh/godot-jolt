...
func _draw_areas() -> void:
	for function_i in function_points.size():
		var fp_augmented: PackedVector2Array = function_points_pos[function_i] \
		...
		for point_i in fp_augmented.size():
			base_color.a = remap(fp_augmented[point_i].y, y_sampled_domain.left, y_sampled_domain.right, 0.0, 0.8)
