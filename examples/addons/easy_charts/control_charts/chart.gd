...
# The RefCounted Rectangle to plot samples
# It is the @bounding_box Rectangle inverted checked the Y axis
var x_sampled_domain: Pair
var y_sampled_domain: Pair


var y_has_decimals: bool
var _y_label_size: Vector2 = Vector2.ZERO # offset only checked the X axis
var _y_label_offset: int = 15 # offset only checked the X axis
var _y_ticklabel_size: Vector2 # offset only checked the X axis
var _y_ticklabel_offset: int = 5 # offset only checked the X axis
var _y_tick_size: int = 7

var x_has_decimals: bool
var _x_label_size: Vector2 = Vector2.ZERO # offset only checked the X axis
var _x_label_offset: int = 15 # offset only checked the X axis
var _x_ticklabel_size: Vector2 # offset only checked the X axis
var _x_ticklabel_offset: int = 5 # offset only checked the X axis
...
func _calc_bounding_box() -> void:
	...
	if chart_properties.labels:
		...
		# calculate the string length of the largest value checked the X axis.
		...
		# calculate the string length of the largest value checked the Y axis.
		# remember that "-" sign adds additional pixels, and it is relative only to negative numbers!
...
func _draw_origin() -> void:
	...
	draw_line(Vector2(xorigin, bounding_box.position.y),Vector2(xorigin, bounding_box.position.y + bounding_box.size.y),Color.BLACK,1)
	draw_line(Vector2(bounding_box.position.x, yorigin),Vector2(bounding_box.position.x + bounding_box.size.x, yorigin),Color.BLACK,1)
