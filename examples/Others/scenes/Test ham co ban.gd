...
## Tinh goc
# deg2rad ( float deg ) doi do ve radian
# rad2deg(s) doi radian ve do
...
# Instance a scene. var diamond = preload("res://diamond.tscn").instance()
...
# to_json ( Variant var ) Converts a Variant var to JSON text and return the result
...
# var2str ( Variant var ) Converts a Variant var to a formatted string that can later be parsed using str2var.
# str2var ( String string ) Converts a formatted string that was returned by var2str to the original value.
...
# linear2db ( float nrg ) lien quan den am thanh
# db2linear ( float db ) lien quan den am thanh
# inst2dict ( Object inst ) lien quan den tu dien
# dict2inst ( Dictionary dict ) lien quan den tu dien
# bytes2var ( PoolByteArray bytes, bool allow_objects=false ) giai ma byte
# weakref ( Object obj ) Returns a weak reference to an object.

# var2bytes ( Variant var, bool full_objects=false ) Encodes a variable value to a byte array.
#	var a = var2bytes(12000)
#	for i in a:
#		print(str(i))
...
# dectime ( float value, float amount, float step ) Returns the result of value decreased by step * amount. a = dectime(60, 10, 0.1)) # a is 59.0
...
## Lam tron
# ceil( float s ) lam tron theo 0.0 giong nhu floor, round, stepify, and int.
# floor ( float s ) lam tron theo 0.5 chuan hon ceil
# stepify ( float s, float step ) lam tron s theo step
# int

## Random
# rand_range ( float from, float to ) tra ngau nhien mot so float trong khoang tu from den to
...
#func _ready() -> void:
#	randomize()
#	$Button.text = str(var2bytes(464,false))
