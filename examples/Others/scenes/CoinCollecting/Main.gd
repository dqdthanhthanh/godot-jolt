...
var Coin = preload("res://TestScene/CoinCollecting/Coin.tscn")
var coin_amount := 0
...
func _unhandled_input(event: InputEvent) -> void:
	if event is InputEventMouseButton:
		if event.is_action_pressed("ui_tapped"):
			var _coin: Sprite2D = Coin.instantiate(PackedScene.GEN_EDIT_STATE_INSTANCE)
			...
			_coin_tween.connect("tween_all_completed", Callable(self, "_on_Coin_update_score").bind(_coin))
			
			_coin_tween.start()

func _get_coin_target_position() -> Vector2:
	var _coin_pos: Vector2 = (score_icon.get_global_transform_with_canvas().origin + (score_icon.size / 2)) * main_camera.zoom
...
func _on_Coin_update_score(coin: Sprite2D) -> void:
	coin_amount += 1
